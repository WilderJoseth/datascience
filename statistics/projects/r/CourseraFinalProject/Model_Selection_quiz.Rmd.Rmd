...
ames_train.1 = ames_train %>%
  select(price, area, Lot.Area, Bedroom.AbvGr, Overall.Qual, Land.Slope)

ames_train.1 = ames_train.1 %>%
  mutate(l_price = log(price),
         l_area = log(area),
         l_Lot.Area = log(Lot.Area))
...
ames.lm = lm(l_price ~ l_area + l_Lot.Area + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train.1)

stepAIC(ames.lm, k = 2)
```

```{r}
stepAIC(ames.lm, k = log(n))
```
...
ames.lm.1 = lm(l_price ~ Bedroom.AbvGr, data = ames_train.1)
summary(ames.lm.1)
```

```{r}
ames.lm.2 = lm(l_price ~ Bedroom.AbvGr + l_area, data = ames_train.1)
summary(ames.lm.2)
```
...
ames_train.2 = ames_train %>%
  select(price, area, Neighborhood)

ames_train.2 = ames_train.2 %>%
  mutate(l_price = log(price),
         l_area = log(area))

ames.lm.2 = lm(l_price ~ l_area, data = ames_train.2)
summary(ames.lm.2)
```

```{r}
ames_train.2$residual = ames.lm.2$residuals
head(ames_train.2)
```
...
ames_train.2 %>%
  ...
  summarise(avg.residuals = mean(residual)) %>%
  filter(avg.residuals == max(avg.residuals))
```
...
ames_train.3 = ames_train %>%
  select(price, Overall.Qual, Bsmt.Qual, Garage.Qual)

ames_train.3 = ames_train.3 %>%
  mutate(l_price = log(price))

summary(ames_train.3)
```

```{r}
# Look for missing values
ames_train.3 %>%
  filter(is.na(Bsmt.Qual) | is.na(Garage.Qual)) %>%
  nrow()
```
...
ames_train.3 = ames_train %>%
  select(price, Overall.Qual, Overall.Cond, MS.SubClass)
...
```{r}
ames.lm.3 = lm(l_price ~ Overall.Qual + Overall.Cond, data = ames_train.3)
summary(ames.lm.3)
```

```{r}
ames_train.3$predicted = ames.lm.3$fitted.values
head(ames_train.3)
```
...
ames_train.3 %>%
  ...
  summarise(avg.predicted = mean(predicted)) %>%
  filter(avg.predicted == max(avg.predicted))
```
...
```{r}
plot(ames.lm.3$residuals ~ ames.lm.3$fitted.values)
```

```{r}
which(lm.influence(ames.lm.3)$hat == max(lm.influence(ames.lm.3)$hat))
```
...
ames_train.4 = ames_train %>%
  select(price, Bedroom.AbvGr)

ames_train.4 = ames_train.4 %>%
  mutate(l_price = log(price))

summary(ames_train.4)
```

```{r}
ames.lm.4 = lm(l_price ~ Bedroom.AbvGr, data = ames_train.4)
summary(ames.lm.4)
```
...
n.no.normal = ames_train %>%
  filter(Sale.Condition == 'Abnorml' | Sale.Condition == 'Partial') %>%
  nrow()
...
ames_train.2.normal = ames_train %>%
  filter(!(Sale.Condition == 'Abnorml' | Sale.Condition == 'Partial')) %>%
  select(price, area, Neighborhood)

ames_train.2.normal = ames_train.2.normal %>%
  mutate(l_price = log(price),
         l_area = log(area))

ames.normal.lm.2 = lm(l_price ~ l_area, data = ames_train.2.normal)
summary(ames.normal.lm.2)
```
