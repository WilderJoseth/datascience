...
ames_test.1 = ames_test %>%
  filter(Sale.Condition == 'Normal')

#ames_test.1 = ames_test.1 %>%
#  mutate(Garage.Area = log(Garage.Area + 1),
...
ames_test.1 = ames_test.1 %>%
  dplyr::select(Overall.Qual, Garage.Area, Total.Bsmt.SF, Garage.Cars, area, Full.Bath, Half.Bath, Bedroom.AbvGr, Year.Built, X1st.Flr.SF, X2nd.Flr.SF, Lot.Area, Central.Air, Overall.Cond)

head(ames_test.1)
```

```{r}
test.predict.1 = predict(model.AIC, ames_test.1)
mean(exp(test.predict.1))
```
