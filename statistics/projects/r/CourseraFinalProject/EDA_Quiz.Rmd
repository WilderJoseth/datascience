...
```{r Q3}
d1 = setNames(aggregate(ames_train$price, list(ames_train$Neighborhood), sd), c('Neighborhood', 'SdPrice'))
d1[order(d1$SdPrice, decreasing = TRUE), ]
```
...
```{r}
ames_train %>%
  select(Lot.Area, Bedroom.AbvGr, Overall.Qual, Year.Built, price) %>%
  cor()
```
...
```{r}
ggplot(ames_train, aes(x = area, y = log(price))) +
  geom_point()
```

```{r}
ggplot(ames_train, aes(x = log(area), y = price)) +
  geom_point()
```
...
ames_train = ames_train %>%
  mutate('hasGarage' = factor(ifelse(Garage.Area > 0, 1, 0), c(0, 1)))
#nrow(filter(ames_train, Garage.Area == 0))
```

```{r}
summary(ames_train$hasGarage)
```
...
```{r}
#setNames(aggregate(ames_train$price, list(ames_train$Bsmt.Qual), sd), c('Bsmt.Qual', 'CountPID'))
# NA - No basement
count(ames_train, Bsmt.Qual)
```

```{r}
count(ames_train, Street)
```
...
inference(y = area, x = hasGarage, data = ames_train, statistic = 'mean', type = 'ht', null = 0, alternative = 'twosided', method = "theoretical")
```
...
ames_bed = ames_train %>%
  filter(area > 2000) %>%
    select(Bedroom.AbvGr)

sum(ames_bed$Bedroom.AbvGr)
```

```{r}
k = 9
k_pos = k + sum(ames_bed$Bedroom.AbvGr)
k_pos
```
...
```{r}
theta = 1/3
theta_pos = theta/(n*theta + 1)
theta_pos
```

```{r}
mean_post = k_pos * theta_pos
mean_post
```

```{r}
theta_pos*sqrt(k_pos)
```
...
lm.simple = lm(l_price ~ l_area, data = ames_train)
summary(lm.simple)
```

```{r}
plot(lm.simple$residuals ~ lm.simple$fitted)
```

```{r}
hist(lm.simple$residuals)
```

```{r}
lm.simple$residuals[lm.simple$residuals < -1]
```

```{r}
ames_train[c(206, 310, 428, 741),] %>%
